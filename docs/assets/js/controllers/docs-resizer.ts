...
(function(){
    ...
    application.register("docs-resizer", class extends Stimulus.Controller {
        ...
        _handleSizeEvent(evt: Event, cls: string) {
            ...
            if (this._selected) {
                this._selected.classList.remove("bg-black-225");
                this._selected.classList.remove("fc-black-500");
            }
            this._sizeClass = cls;
            this.resizableTarget.classList.add(cls);
            this._selected = evt.currentTarget as Element;
            evt.currentTarget.classList.add("bg-black-225");
            evt.currentTarget.classList.add("fc-black-500");
            evt.preventDefault();
        }
    });
})();