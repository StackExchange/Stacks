(function () {
    ...
    Stacks.addController("s-sortable-column", {
        ...
        setCurrentSort: function (direction) {
            ...
            $(this.element)
                .find(".js-sorting-indicator").addClass("d-none").end()
                .find(".js-sorting-indicator-" + direction).removeClass("d-none").end()
                .toggleClass("is-sorted", direction !== "none");
            if (direction === "none") {
        ...
        sort: function (evt) {
            ...
            var allExceptThis = $table.find("[data-controller~='s-sortable-column']").toArray().map(function (elem) {
            ...
            var slotIndex = buildIndex($table.find("> tbody")[0]);
            ...
            var data = rows.map(function (row, index) {
                ...
                var explicit = $cell.attr("data-sort-val");
                var d = typeof explicit === "string" ? explicit : $cell.text().trim();
            ...
            data.forEach(function (tup) {
                $(rows[tup[1]]).appendTo($table.find("> tbody"));
            });
