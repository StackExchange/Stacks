(function () {
    ...
    Stacks.addController("s-sortable-column", {
        ...
        setCurrentSort: function (direction) {
            ...
            this.element.querySelectorAll(".js-sorting-indicator").forEach(function (el) { // TODO polyfill forEach for IE11
                el.classList.toggle("d-none", !el.classList.contains("js-sorting-indicator-" + direction)); // TODO polyfill two-argument version of toggle for IE11
            });
        ...
        sort: function (evt) {
            var controller = this;
            var table = this.element.closest("table"); // TODO polyfill closest for IE11
            var tbody = table.querySelector(":scope > tbody");

            // TODO polyfill Array.from for IE11
            allExceptThis = Array.from(table.querySelectorAll("[data-controller~='s-sortable-column']")).map(function (elem) {
        ...
        while (curRow || growingRowsLeft.some(function (e) { return e; })) {
            ...
            if (curRow) {
                for (var curCellInd = 0; curCellInd < curRow.children.length; curCellInd++) {
                    ...
                    nextFreeSlot = curSlot + cell.colSpan;
